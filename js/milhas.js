var MilhasModule = {
    ...
    renderVisao() {
        ...
        // Ocultar clientes sem movimentação
    ...
    portalCliente(clienteId) {
        ...
        // Cabeçalho com destaque para o nome
        let html = `
            <div style="background:#fff;border:1px solid var(--border);border-bottom:3px solid var(--primary);padding:20px;border-radius:8px;margin-bottom:20px;">
                <h2 style="margin:0 0 10px;font-size:24px;font-weight:700;color:var(--primary);">${AppModule.escapeHtml(c.nome)}</h2>
        ...
        // Cards de resumo (clean, 4 por linha)
        html += `
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px;">
                <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:16px;text-align:center;">
                    <label style="display:block;font-size:12px;color:var(--text-muted);margin-bottom:8px;">✈️ Vôos</label>
                    <span style="font-size:26px;font-weight:700;color:var(--primary);">${totalVoos}</span>
                </div>
                <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:16px;text-align:center;">
                    <label style="display:block;font-size:12px;color:var(--text-muted);margin-bottom:8px;">💎 Valor de Mercado</label>
                    <span style="font-size:20px;font-weight:700;color:var(--text);">${AppModule.formatCurrency(totalValorMercado)}</span>
                </div>
                <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:16px;text-align:center;">
                    <label style="display:block;font-size:12px;color:var(--text-muted);margin-bottom:8px;">💰 Economia</label>
        ...
        html += `<h4 style="margin:20px 0 10px;font-size:16px;color:var(--text);border-bottom:2px solid var(--primary);padding-bottom:8px;">🎯 Programas de Fidelidade (${programas.length})</h4>`;
        if (!programas.length) {
            ...
            html += programas.map(p => {
                ...
                return `
                    <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;gap:8px;">
                        <div>
                            <strong>${AppModule.escapeHtml(p.programa)}</strong>
                        </div>
                        <div style="text-align:right;">
                            <span style="font-weight:600;color:var(--primary);">${this.fmtMilhas(saldo)} milhas</span>
                            ${variacao !== 0 ? `<div style="font-size:12px;color:${variacao >= 0 ? 'var(--success)' : 'var(--danger)'};">${variacao >= 0 ? '+' : ''}${this.fmtMilhas(variacao)}</div>` : ''}
                        </div>
                    </div>
                `;
            }).join('');
        ...
        // Cartões
        html += `<h4 style="margin:20px 0 10px;font-size:16px;color:var(--text);border-bottom:2px solid var(--primary);padding-bottom:8px;">💳 Cartões (${cartoes.length})</h4>`;
        if (!cartoes.length) {
            ...
            html += cartoes.map(cartao => `
                <div style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;gap:8px;">
                    <div>
                        ...
                        <strong style="margin-left:6px;">${AppModule.escapeHtml(cartao.banco)}</strong>
                        ${cartao.nome ? `<div style="font-size:12px;color:var(--text-muted);margin-top:4px;">${AppModule.escapeHtml(cartao.nome)}</div>` : ''}
                    </div>
        ...
        // Emissões
        html += `<h4 style="margin:20px 0 10px;font-size:16px;color:var(--text);border-bottom:2px solid var(--primary);padding-bottom:8px;">🎫 Emissões (${emissoes.length})</h4>`;
        if (!emissoes.length) {
            ...
            html += `
                <div class="table-wrap">
                    <table class="table">
                        <thead>
                            <tr><th>Data</th><th>Rota</th><th>Valor Mercado</th><th>Valor Emitido</th><th>Economia</th></tr>
                        </thead>
                        <tbody>
        ...
        AppModule.openModal(`Portal do Cliente — ${c.nome}`, html, footer);
    },
