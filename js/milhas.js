var MilhasModule = {
    ...
    getClienteNome(clienteId) {
        return DB.getClienteNome ? DB.getClienteNome(clienteId) : '';
    },
    ...
    getSaldoAtual(programa) {
        ...
        if (h.length) return parseInt(h[h.length - 1].saldo) || 0;
        return parseInt(programa.saldoInicial) || 0;
    },

    getVariacao(programa) {
        const h = programa.historico || [];
        if (h.length < 2) return 0;
        const atual = parseInt(h[h.length - 1].saldo) || 0;
        const anterior = parseInt(h[h.length - 2].saldo) || 0;
    ...
    getValorMercadoCliente(clienteId) {
        return this.getEmissoesCliente(clienteId)
            .reduce((s, e) => s + (parseFloat(e.valorMercado) || 0), 0);
    },
    ...
    clienteTemMovimentacao(clienteId) {
        return this.getProgramasCliente(clienteId).length > 0
            || this.getCartoesCliente(clienteId).length > 0
            || this.getEmissoesCliente(clienteId).length > 0
            || this.getViagensCliente(clienteId).length > 0;
    },
    ...
    renderVisao() {
        ...
        let kpis = `
            <div class="kpi-grid">
                ...
                <div class="kpi-card"><label>Clientes com Movimentação</label><span>${clientes.filter(c => this.clienteTemMovimentacao(c.id)).length}</span></div>
            </div>
        `;

        const clientesComMov = clientes.filter(c => this.clienteTemMovimentacao(c.id));

        if (!clientesComMov.length) {
            panel.innerHTML = kpis + '<p class="dashboard-empty" style="margin-top:16px;">Nenhum cliente com movimentação.</p>';
            return;
        }

        const cards = clientesComMov.map(c => {
            ...
            return `
                <div class="cliente-card" style="margin-bottom:12px;">
                    <div class="cliente-card-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px;">
                        <div>
        ...
        panel.innerHTML = kpis + `<div class="cliente-card-grid" style="margin-top:16px;">${cards}</div>`;
    },
    ...
    portalCliente(clienteId) {
        ...
        if (!programas.length) {
            ...
            html += programas.map(p => {
                ...
                const ultimaAtualizacao = p.historico && p.historico.length > 0
                    ? this.formatDateBR(p.historico[p.historico.length - 1].data)
                    : '—';
                return `
        ...
        if (!emissoes.length) {
            ...
            html += emissoes.map(e => {
                ...
                return `
                    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:12px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                        <div>
                            <div style="font-size:14px;font-weight:600;">${this.formatDateBR(e.data)} • ${AppModule.escapeHtml(e.origem || '')} → ${AppModule.escapeHtml(e.destino || '')}</div>
                            <div style="font-size:12px;color:var(--text-muted);margin-top:2px;">Milhas: ${this.fmtMilhas(e.milhasUtilizadas)} | Custo milha: ${AppModule.formatCurrency(e.custoMilha)}</div>
                        </div>
        ...
        if (!viagens.length) {
            ...
            html += viagens.map(v => `
                <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:12px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                    <div>
                        <div style="font-size:14px;font-weight:600;">${this.formatDateBR(v.dataIda)} • ${AppModule.escapeHtml(v.destino || '')}</div>
                        ...
                        ${programas.map(p => {
                            ...
                            return `
                                <tr>
                                    ...
                                    <td><strong>${AppModule.escapeHtml(p.programa || p.nome || '')}</strong></td>
    ...
    abrirFormPrograma(programa = null) {
        ...
        const html = `
            ...
            <div class="form-group"><label>Programa *</label>
                <input type="text" id="prg-programa" class="form-control" list="dl-programas" value="${AppModule.escapeHtml(programa?.programa || programa?.nome || '')}">
            </div>
            ${!isEdit ? `
                <div class="form-grid">
    ...
    atualizarSaldo(id) {
        ...
        AppModule.openModal(`Atualizar saldo — ${p.programa || p.nome}`, html, footer);
    },

    salvarSaldo(id) {
        const mes = document.getElementById('sal-mes').value;
        const novoSaldo = parseInt(document.getElementById('sal-novo').value);
    ...
    verHistorico(id) {
        ...
        AppModule.openModal(`Histórico — ${p.programa || p.nome} (${this.getClienteNome(p.clienteId)})`, html, footer);
    },
    ...
    renderEmissoes() {
        ...
        emissoes.forEach(e => {
            ...
            totalMilhas += parseInt(e.milhasUtilizadas) || 0;
        });
    ...
    atualizarProgramasEmissao(programaSelecionado = '') {
        ...
        select.innerHTML = programas.length
            ? programas.map(p => `<option value="${AppModule.escapeHtml(p.programa || p.nome)}" ${programaSelecionado === (p.programa || p.nome) ? 'selected' : ''}>${AppModule.escapeHtml(p.programa || p.nome)}</option>`).join('')
    ...
    salvarEmissao(id) {
        ...
        const milhas = parseInt(document.getElementById('ems-milhas').value);
