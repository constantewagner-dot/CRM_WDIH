const RelatoriosModule = {
    ...
    render() {
        ...
        const vendas = this.filtrarPorPeriodo(DB.get('vendas', []), range, 'data');
    ...
    calcularPeriodo() {
        ...
        switch (this.periodoAtual) {
            ...
            case 'custom':
                ...
                return {
                    inicio: di ? new Date(di + 'T00:00:00') : null,
                    fim: df ? new Date(df + 'T23:59:59') : null
                };
            case 'todos':
            default:
    ...
    renderKPIs(vendas, negocios, clientes) {
        ...
        const fechados = negocios.filter(n => n.etapa === 'Fechado (Ganho)').length;
        const perdidos = negocios.filter(n => n.etapa === 'Perdido').length;
    ...
    renderReceitaPorMes(vendas) {
        ...
        vendas.forEach(v => {
            ...
            grupos[chave] = (grupos[chave] || 0) + (parseFloat(v.valor_total) || 0);
        });

        const chaves = Object.keys(grupos).sort();
        if (!chaves.length) {
            container.innerHTML = '<p class="dashboard-empty">Sem vendas no período.</p>';
            return;
        }
    ...
    renderReceitaPorServico(vendas) {
        ...
        vendas.forEach(v => {
            const s = v.servico || '—';
            grupos[s] = (grupos[s] || 0) + (parseFloat(v.valor_total) || 0);
        });
    ...
    renderTopClientes(vendas) {
        ...
        const grupos = {};
        vendas.forEach(v => {
            const nome = v.cliente_nome || DB.getClienteNome(v.cliente_id) || '—';
            grupos[nome] = (grupos[nome] || 0) + (parseFloat(v.valor_total) || 0);
        });
        ...
        container.innerHTML = top.map(([nome, valor], i) => `
            <div class="list-item">
                <div class="list-item-info">
                    <h4>${i + 1}. ${AppModule.escapeHtml(nome)}</h4>
                </div>
    ...
    renderPipeline(negocios) {
        ...
        negocios.forEach(n => {
            if (contagem[n.etapa] !== undefined) contagem[n.etapa]++;
        });
