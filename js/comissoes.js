    ...
    render() {
        ...
        list.innerHTML = comissoes.length
            ...
            <div style="margin-bottom:16px;padding:12px;background:var(--gray-50);border-radius:8px;">
                <strong>Total de Comissões (${percentual}%):</strong> ${AppModule.formatCurrency(totalComissoes)}
            </div>
            <table class="table">
                <thead>
                    <tr>
                ...
                <tbody>
                    ${comissoes.map(c => `
                        <tr>
                            <td>${c.cliente || '—'}</td>
                            <td>${c.servico || '—'}</td>
                            <td>${AppModule.formatCurrency(c.valor)}</td>
                            <td><strong>${AppModule.formatCurrency(c.comissao)}</strong></td>
                            <td>${c.data || '—'}</td>
                        </tr>
                    `).join('')}
                </tbody>
